...
void initTmpl(CylinderTmpl *Tmpl)
{
    ...
    for (i=2;i<32;i=i+1)
     {
        Tmpl->zoneR[i]=(i-1)*50; Tmpl->sectorN[i]=2*M_PI*((i-1)*50)/50;
     }

     for (i=32;i<43;i=i+1)
     {
         Tmpl->zoneR[i]=1500+(i-31)*100; Tmpl->sectorN[i]=2*M_PI*(1500+(i-31)*100)/100;
     }

     for (i=43;i<56;i=i+1)
     {
         Tmpl->zoneR[i]=2600+(i-42)*200; Tmpl->sectorN[i]=2*M_PI*(2400+(i-42)*200)/250;
     }

     for (i=56;i<66;i=i+1)
     {
         Tmpl->zoneR[i]=5200+(i-55)*550; Tmpl->sectorN[i]=2*M_PI*(5000+(i-55)*550)/750;
     }

     for (i=66;i<77;i=i+1)
     {
         Tmpl->zoneR[i]=10700+(i-65)*1000; Tmpl->sectorN[i]=2*M_PI*(10000+(i-65)*1000)/1000;
     }
   
     for (i=77;i<97;i=i+1)
     {
         Tmpl->zoneR[i]=21700+(i-76)*1500; Tmpl->sectorN[i]=2*M_PI*(20000+(i-76)*1500)/1500;
     }

    for (i=97;i<125;i=i+1)
    {
        Tmpl->zoneR[i]=51700+(i-96)*2500; Tmpl->sectorN[i]=2*M_PI*(50000+(i-96)*2500)/2500;
    }

    for (i=125;i<134;i=i+1)
    {
        Tmpl->zoneR[i]=121700+(i-124)*5000; Tmpl->sectorN[i]=2*M_PI*(120000+(i-124)*5000)/5000;
    }
...
void help()
{
	...
	fprintf(stderr,"a resolution of 0.5 second. The partitioning template was based on \n");
	...
	fprintf(stderr,"The partitioning template was based on the tc-cylinder template.\n\n");
	fprintf(stderr,"./tc_cylinder testpois.lfH -GtestDEM15s.grd -ItestDEM3s.grd -TS -Z70 -p10 -r1s \n");
	fprintf(stderr,"Test Data Usage\n\n");
	fprintf(stderr,"If 3 arc second resolution is desired in the inner region and 15 arc second resolution\n");
	fprintf(stderr,"in the outer when calculating the correction values for the calculation points\n");
	fprintf(stderr,"on a profile in the Everest region, the following form can be used.\n");
	fprintf(stderr,"Up to zone 56 is considered the inner zone. The interpolation region is preferred\n");
	fprintf(stderr,"up to the 3th zone at a resolution of 0.5 arc second.\n");
	fprintf(stderr,"The partitioning template was based on the tc-cylinder template.\n\n");
	fprintf(stderr,"./tc_cylinder testpois.lfH -GtestDEM15s.grd -ItestDEM3s.grd -TS -Z56 -p3 -r0.5s > testoutput.txt \n");
}
